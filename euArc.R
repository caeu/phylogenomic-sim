...
dimMeans <- function(arr, dims) {
  # colMeans will sum over the (first:n) dimensions of an array whereas rowMeans will sum over the  (n+1:last) dimensions of an array
  n <- length(dim(arr))
  b <- aperm(arr, c(dims, seq_len(n)[-dims]))
  colMeans(b, dims = length(dims))
}

dimSums <- function(arr, dims) {
  # colSums will remove the first n dimensions of an array whereas rowSums will remove the last n dimensions of an array
  n <- length(dim(arr))
  b <- aperm(arr, c(dims, seq_len(n)[-dims]))
  colSums(b, dims = length(dims))
}
